...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin, mockUser }) => {
  const [loginMethod, setLoginMethod] = useState<'account' | 'magic'>('account');
  ...
  const on2FASubmit = (data: TwoFactorForm) => {
    // In production, validate the 2FA code
    onLogin(mockUser);
  };
  ...
  const handleMagicLinkSubmit = async (data: { email: string }) => {
    // In production, this would trigger the SuperTokens magic link flow
